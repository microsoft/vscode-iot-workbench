...
suite('IoT Device Workbench: Config', () => {
  test('should set and get config value correctly', async function() {
    ...
    assert.strictEqual(ConfigHandler.get<string>(ConfigKey.boardId), 'devkit');
    assert.strictEqual(
      ConfigHandler.get<string>(ConfigKey.devicePath),
      'Device'
    );
    await ConfigHandler.update(ConfigKey.boardId, 'IoTButton');
    assert.strictEqual(
      ConfigHandler.get<string>(ConfigKey.boardId),
      'IoTButton'
    );
