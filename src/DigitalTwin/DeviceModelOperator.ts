...
import {
  MetaModelType,
  humanReadableMetaModelType,
} from './DigitalTwinApi/DataContracts/DigitalTwinContext';
...
export class DeviceModelOperator {
  ...
  private async InitializeFolder(): Promise<string | null> {
    ...
    if (
      ...
      rootPath = await utils.selectWorkspaceItem(
        `Select the folder that will contain your ${DigitalTwinConstants.productName} models:`,
        {
  ...
  async Load(
    ...
    const deviceModelFile = files.find(
      fileName =>
        fileName.endsWith(DigitalTwinConstants.interfaceSuffix) ||
  ...
  async CreateInterface(
    ...
    const interfaceTemplate = context.asAbsolutePath(
      path.join(
    ...
    try {
      const content = fs
        .readFileSync(interfaceTemplate, 'utf8')
        .replace(DigitalTwinConstants.dtidPlaceholder, dtid);
      fs.writeFileSync(targetInterface, content);
    } catch (error) {
      throw new Error(
        `Creating ${DigitalTwinConstants.productName} Interface failed: ${error.message}`
      );
    }
    ...
    vscode.window.showInformationMessage(
      `New ${DigitalTwinConstants.productName} Interface '${dtid}' was created successfully.`
    );
    return;
  }
  ...
  async CreateCapabilityModel(
    ...
    const capabilityModel = context.asAbsolutePath(
      path.join(
        FileNames.resourcesFolderName,
        FileNames.templatesFolderName,
        DigitalTwinFileNames.devicemodelTemplateFolderName,
    ...
    try {
      const content = fs
        .readFileSync(capabilityModel, 'utf8')
        .replace(DigitalTwinConstants.dtidPlaceholder, dtid);
      fs.writeFileSync(targetCapabilityModel, content);
    } catch (error) {
      throw new Error(
        `Creating ${DigitalTwinConstants.productName} Capability Model failed: ${error.message}`
      );
    }
    ...
    vscode.window.showInformationMessage(
      `New ${DigitalTwinConstants.productName} Capability Model '${dtid}' was created successfully.`
    );
    return;
  }
  ...
  async ConnectModelRepository(
    ...
    if (repoSelection.label === 'Public repository') {
      ...
      DeviceModelOperator.vscexpress =
        DeviceModelOperator.vscexpress ||
  ...
  async Disconnect(
    ...
    const message = `Sign out ${DigitalTwinConstants.productName} Model Repository successfully`;
  ...
  async GetInterfaces(
    ...
    if (usePublicRepository) {
      ...
      const connectionString = await CredentialStore.getCredential(
        ConfigKey.modelRepositoryKeyName
      );
      if (!connectionString) {
        vscode.window.showWarningMessage(
          `Failed to get interfaces from ${DigitalTwinConstants.productName} Model Repository. Please sign out and sign in with a valid connection string.`
        );
        return;
      }
  ...
  async GetCapabilityModels(
    ...
    if (usePublicRepository) {
      const dtMetamodelRepositoryClient = new DigitalTwinMetamodelRepositoryClient();
      await dtMetamodelRepositoryClient.initialize(null);
      const result = await dtMetamodelRepositoryClient.SearchCapabilityModelsAsync(
        searchString,
        continueToken,
        undefined,
        pageSize
      ...
      if (!connectionString) {
        vscode.window.showWarningMessage(
          `Failed to get Capability Models from ${DigitalTwinConstants.productName} Model Repository. Please sign out and sign in with a valid connection string.`
        );
        return;
      }
      const dtMetamodelRepositoryClient = new DigitalTwinMetamodelRepositoryClient();
  ...
  async DeleteMetamodelFiles(
    ...
    if (!connectionString) {
      vscode.window.showWarningMessage(
        `Failed to delete models from ${DigitalTwinConstants.productName} Model Repository. Please sign out and sign in with a valid connection string.`
      );
    ...
    for (const id of fileIds) {
      const message = `${DigitalTwinConstants.dtPrefix} Start deleting ${metaModelValue} with id ${id}.`;
      utils.channelShowAndAppendLine(channel, message);
      try {
        if (metaModelType === MetaModelType.Interface) {
          ...
          const message = `${DigitalTwinConstants.dtPrefix} Deleting Interface with id ${id} completed.`;
          utils.channelShowAndAppendLine(channel, message);
        } else {
          ...
          const message = `${DigitalTwinConstants.dtPrefix} Deleting capabilty model with id ${id} completed.`;
          utils.channelShowAndAppendLine(channel, message);
        }
      } catch (error) {
        const message = `${DigitalTwinConstants.dtPrefix} Deleting ${metaModelValue} with id ${id} failed. Error: ${error.message}`;
        utils.channelShowAndAppendLine(channel, message);
      }
    }
  }
  ...
  async DownloadAndEditMetamodelFiles(
    ...
    if (!fileIds || fileIds.length === 0) {
      const message = `${DigitalTwinConstants.dtPrefix} No ${DigitalTwinConstants.productName} model is selected`;
      utils.channelShowAndAppendLine(channel, message);
      return;
    }
    ...
    if (!usePublicRepository) {
      const repoConnectionString = await CredentialStore.getCredential(
        ConfigKey.modelRepositoryKeyName
      );
    ...
    const readableMetaModelValue = humanReadableMetaModelType.get(
      metaModelType
    ) as string;
    for (const id of fileIds) {
      const message = `${DigitalTwinConstants.dtPrefix} Start getting ${readableMetaModelValue} with id ${id}.`;
      utils.channelShowAndAppendLine(channel, message);
      ...
      try {
        if (metaModelType === MetaModelType.Interface) {
          ...
          fileMetaData = await dtMetamodelRepositoryClient.GetCapabilityModelAsync(
            id,
            repositoryId,
            true
          );
        ...
        if (fileMetaData) {
          ...
          const message = `${DigitalTwinConstants.dtPrefix} Downloading ${readableMetaModelValue} with id ${id} into ${candidateName} completed.`;
          utils.channelShowAndAppendLine(channel, message);
        }
      } catch (error) {
        const message = `${DigitalTwinConstants.dtPrefix} Downloading ${readableMetaModelValue} with id ${id} failed. Error: ${error.message}`;
        utils.channelShowAndAppendLine(channel, message);
      }
    }
  ...
  async SubmitMetaModelFiles(
    ...
    if (fileItems.length === 0) {
      const message = `No ${DigitalTwinConstants.productName} models found in current folder. Please select the folder that contains ${DigitalTwinConstants.productName} models and try again.`;
    ...
    const unsavedFiles = vscode.workspace.textDocuments.filter(
      file => file.isDirty
    );

    // Is there any unsaved files to be submitted?
    const filterResult = unsavedFiles.filter(file =>
      selectedFiles.some(
        ({ label, description }) =>
    ...
    if (filterResult.length > 0) {
      const unsavedFileList = filterResult
        .map(file => path.basename(file.fileName))
        .toString();
      const messge = `The following file(s) contain unsaved changes: ${unsavedFileList}, do you want to save them?`;
    ...
    if (!connectionString) {
      utils.channelShowAndAppendLine(
        channel,
        `Company repository not specified, cancel submit.`
      );
      return false;
    }
    const result = await DigitalTwinConnector.ConnectMetamodelRepository(
    ...
    if (!result) {
      utils.channelShowAndAppendLine(
        channel,
        `Failed to connect Company repository, cancel submit.`
      );
      return false;
    }
    ...
    utils.channelShowAndAppendLine(
      channel,
      `${DigitalTwinConstants.dtPrefix} All submitted.`
    );
    return true;
  }

  private async RetrieveModelRepoConnectionString(): Promise<string | null> {
    let connectionString = await CredentialStore.getCredential(
      ConfigKey.modelRepositoryKeyName
    );
    if (!connectionString) {
      const option: vscode.InputBoxOptions = {
  ...
  private async SubmitInterface(
    ...
    let message = `${DigitalTwinConstants.dtPrefix} Start uploading interface '${fileInterface.id}'(${fileInterface.filePath})...`;
    utils.channelShowAndAppendLine(channel, message);
    ...
    if (interfaceMetaData) {
      ...
      message = `The server already has a interface named '${fileInterface.id}'`;
    ...
    message = `Interface '${fileInterface.id}'(${fileInterface.filePath}) has been successfully submitted.`;
    utils.channelShowAndAppendLine(channel, `  ${message}`);
    vscode.window.showInformationMessage(message);
  }
  ...
  private async SubmitCapabilityModel(
    ...
    let message = `${DigitalTwinConstants.dtPrefix} Start uploading device capability model '${fileDCM.id}'(${fileDCM.filePath})...`;
    utils.channelShowAndAppendLine(channel, message);
    ...
    try {
      // Try to get the file to retrieve the latest etag.
      capabilityModelContext = await dtMetamodelRepositoryClient.GetCapabilityModelAsync(
        fileDCM.id,
        builder.RepositoryIdValue,
        true
      );
    ...
    if (capabilityModelContext) {
      // Update exiting DCM
      message = `The server already has a device capability model named '${fileDCM.id}'`;
    ...
    message = `Device capability model '${fileDCM.id}'(${fileDCM.filePath}) has been successfully submitted.`;
    utils.channelShowAndAppendLine(channel, `  ${message}`);
    vscode.window.showInformationMessage(message);
  }
}
