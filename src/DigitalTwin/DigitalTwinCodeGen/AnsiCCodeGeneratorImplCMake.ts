...
export class AnsiCCodeGeneratorImplCMake extends AnsiCCodeGeneratorBase {
  ...
  async GenerateCode(
    ...
    for (const fileInfo of templateFilesInfo) {
      ...
      if (fileInfo.fileName === 'CMakeLists.txt') {
        ...
        // CMakeLists.txt
        const includedHeaderFiles = this.getIncludedFileListString(
          generatedFiles,
          targetPath,
          '.h'
        );
        const includedCFiles = this.getIncludedFileListString(
          generatedFiles,
          targetPath,
          '.c'
        );
  ...
  getIncludedFileListString(
    ...
    if (generatedFiles !== null && generatedFiles.length > 0) {
      const includeFiles = generatedFiles
        .filter(
          file =>
            path.extname(file) === fileExtension && !file.endsWith('main.c')
        )
