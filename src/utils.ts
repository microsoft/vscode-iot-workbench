...
} from './DigitalTwin/DigitalTwinCodeGen/Interfaces/CodeGenerator';
...
export function getScriptTemplateNameFromLanguage(
  language: string
): string | undefined {
  switch (language) {
 ...
 */
export interface FolderQuickPickItem<T = undefined>
...
export async function selectWorkspaceItem(
  ...
  if (vscode.workspace.workspaceFolders) {
    folderPicks = vscode.workspace.workspaceFolders.map(
  ...
  folder = await vscode.window.showQuickPick(folderPicks, {
    placeHolder,
    ignoreFocusOut: true,
  });
  if (folder === undefined) {
...
export async function askAndOpenInRemote(
  ...
  const message = `${operation} can only be executed in remote container. Do you want to reopen the IoT project in container?`;
  const result:
    | vscode.MessageItem
    | undefined = await vscode.window.showInformationMessage(
    message,
  ...
  if (result === DialogResponses.yes) {
    telemetryContext.properties.errorMessage = `${operation} Operation failed and user opens project in container.`;
    const res = await RemoteExtension.checkRemoteExtension();
    if (!res) {
      telemetryContext.properties.errorMessage = `${operation} Operation failed on installing Remote Extension.`;
...
export async function TakeNoDeviceSurvey(telemetryContext: TelemetryContext) {
  ...
  if (result === DialogResponses.yes) {
    ...
    await vscode.commands.executeCommand(
      'vscode.open',
      vscode.Uri.parse(
        `${noDeviceSurveyUrl}?o=${encodeURIComponent(
          process.platform
...
export async function getTemplateFilesInfo(
  ...
  templateFilesJson.templateFiles.forEach((fileInfo: TemplateFileInfo) => {
    ...
    templateFilesInfo.push({
      ...
      overwrite:
        typeof fileInfo.overwrite !== 'undefined' ? fileInfo.overwrite : true, // if it is not defined, we will overwrite the existing file.
      fileContent,
    });
  });
...
export async function GetCodeGenTemplateFolderName(
  ...
  const templateFilePath = context.asAbsolutePath(
    path.join(
  ...
  const templateFile = (await FileUtility.readFile(
    ScaffoldType.Local,
    templateFilePath,
    'utf8'
  )) as string;
  ...
  const result = templateFileJson.templates.filter(
...
export async function generateTemplateFile(
  ...
  if (fileInfo.fileContent) {
    try {
      ...
      throw new Error(
        `Failed to create sketch file ${fileInfo.fileName}: ${error.message}`
      );
    }
  }
  return true;
...
export async function handleIoTWorkspaceProjectFolder(
  ...
  const workspaceFiles = fs
    ...
    .filter(file =>
      path.extname(file).endsWith(FileNames.workspaceExtensionName)
    );
